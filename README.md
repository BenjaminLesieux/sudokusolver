...
We did 3 differents tests.
- a grid have a solution
```Scala
...
```
- an incorrect grid cannot have a solution
```Scala
...
```
- an unsolvable grid cannot have a solution
```Scala
"An unsolvable grid" should "not have a solution" in {
    ...
    assertThrows[IllegalArgumentException](unsolvableSudokuGrid.solve())
  }
```
